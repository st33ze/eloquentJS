...
async function locateScalpel(nest){
  ...
  try {
    lastLoc = await storage(nest, "scalpel");

    while(currentLoc !== lastLoc) {
      currentLoc = lastLoc;
      lastLoc = await anyStorage(nest, currentLoc, "scalpel");
    }
...
function locateScalpel2(nest) {
  return new Promise((resolve, reject) => {
    let lastLoc = nest.name;

    function checkLocation(){
      return anyStorage(nest, lastLoc, "scalpel").then(function(location){
        if (location == lastLoc) resolve(location);
        lastLoc = location;
        checkLocation();
...
locateScalpel(bigOak).then(consle.log);
...
function Promise_all(promises) {
  return new Promise((resolve, reject) => {
    ...
    while (promises.length != results.length)
    resolve(results);
  });
}
