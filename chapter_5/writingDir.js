...
function dominantDirection(text) {
    ...
    for (let char of text) {
        let foundDir = findDirection(char);
        if (foundDir) {}
        let knownDir = directions.findIndex(d => d.name == foundDir);
        if (foundDir) {
            if (knownDir == -1) directions.push({name: foundDir, count: 1});
            else directions[knownDir].count++;
        }
    }
    
    // Return most popular direction.
    return directions.reduce((mostPopular, direction) => {
        return mostPopular.count > direction.count ? mostPopular: direction;
    }, {}).name;
}

function findDirection(char) {
    let cCode = char.codePointAt(0);

    for (let script of SCRIPTS) {
        if (script.ranges.some(([from, to]) => {
            return cCode >= from && cCode < to;
        })) {
